...
if ! ping -c 1 -W 2 192.168.1.180 >/dev/null 2>&1; then
    echo "Incus host is already unreachable/powered off."
    "$SCRIPT_DIR/log-power-event.sh" "Shutdown" "Automation" "Success" "Incus already offline."
    exit 0
fi
