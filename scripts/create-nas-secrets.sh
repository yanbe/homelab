...
# 4. SSH CA鍵ペアの作成
if [ ! -f "$HOME/ssh-ca/ssh_ca_key" ]; then
	mkdir -p "$HOME/ssh-ca"
	ssh-keygen -t ed25519 -f "$HOME/ssh-ca/ssh_ca_key" -C "NAS_SSH_CA" -N ""
fi
cp "$HOME/ssh-ca/ssh_ca_key.pub" ./secrets/nas/etc/ssh/trusted-user-ca-keys.pem
